      ...
      <div className="grid grid-cols-7 w-full border-b border-gray-200 bg-white sticky top-0 z-10 divide-x divide-gray-100">
        {weekDays.map((day) => {
          ...
          return (
            ...
            >
              <span className={`text-xs font-medium ${isDaySelected ? 'text-blue-600' : 'text-gray-500'}`}>
                {format(day, 'EEE')}
              </span>
              <span className={`text-lg ${isDaySelected ? 'font-bold text-blue-600' : ''}`}>
                {format(day, 'd')}
              </span>
              {isDayToday && (
                <span className={`text-xs ${isDaySelected ? 'text-blue-600' : 'text-gray-500'} font-medium mt-1`}>
                  Today
                </span>
              )}
              {isDayYesterday && (
        ...
        <div className="bg-white mb-3 shadow-sm">
          ...
          <div className="divide-y divide-gray-100">
            {allDayTodos.length > 0 ? (
              ...
              <div className="p-3 text-gray-500 text-center">No all-day tasks</div>
            )}
