    ...
    return (
      <div className="grid grid-cols-7 w-full border-b border-gray-200 bg-white sticky top-0 z-10 divide-x divide-gray-100">
        {months.map((month, index) => (
          ...
          >
            <span className={`text-xs font-medium ${month.isCurrent ? 'text-blue-600' : 'text-gray-500'}`}>
              {month.year}
            </span>
            <span className={`text-lg ${month.isCurrent ? 'font-bold text-blue-700' : 'font-medium text-gray-600'}`}>
              {month.month}
            </span>
            {month.isPrevious && (
